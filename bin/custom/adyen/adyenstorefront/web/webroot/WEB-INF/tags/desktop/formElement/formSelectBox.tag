			...
			<div class="controls">
				<form:select id="${idKey}" path="${path}" cssClass="${selectCSSClass}" tabindex="${tabindex}">
					<c:if test="${skipBlank == null || skipBlank == false}">
						<option value="" <c:if test="${mandatory != null && mandatory == true}">disabled="disabled"</c:if> ${empty selectedValue ? 'selected="selected"' : ''}><spring:theme code='${skipBlankMessageKey}'/></option>
					</c:if>
