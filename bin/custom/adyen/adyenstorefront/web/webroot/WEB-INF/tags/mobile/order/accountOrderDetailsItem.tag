	...
	<c:when test="${not inProgress}">
		<div class="productItemHeader">
			<c:if test="${consignment.status.code eq 'READY_FOR_PICKUP'}" >
				<span class="pick-up-reminder"><spring:theme code="text.accout.order.warning.storePickUpItems" text="Reminder - Please pick up your items(s) soon."/></span>
			</c:if>
			<h3>${consignment.statusDisplay}: <fmt:formatDate value="${consignment.statusDate}" pattern="MM/dd/yy"/></h3>
			<c:choose>
